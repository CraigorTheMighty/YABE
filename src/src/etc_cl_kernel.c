...
"                    dist[2] = (int)MEMBER(l1_quant, Z) - (int)MEMBER(l0_quant, Z);\n"\
"\n"\
"                    if (dist[0]*dist[0] > 9 || dist[1]*dist[1] > 9 || dist[2]*dist[2] > 9)\n"\
"                        continue;\n"\
"\n"\
"                    local_error = ETC_EvaluateError(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo5Bit(l0_quant), &g_partition_indexes[flipped][0][0]);\n"\
"                    local_error += ETC_EvaluateError(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo5Bit(l1_quant), &g_partition_indexes[flipped][1][0]);\n"\
...
"            for (partition = 0; partition < 2; partition++)\n"\
"            {\n"\
"                const int neighbourhood_width = 2;\n"\
"                float best_err = MAX_INIT_ERROR;\n"\
"                int x;\n"\
"                int y;\n"\
"                int z;\n"\
...
"            {\n"\
"                etc_sample_t sample;\n"\
"                sample.pos_quantised = UCHAR3(0, 0, 0);\n"\
"                DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"                pathing_params.partition_indexes = &g_partition_indexes[flipped][partition][0];\n"\
"                sample.error = ETC_EvaluateError(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo5Bit(sample.pos_quantised), &g_partition_indexes[flipped][partition][0]);\n"\
"                DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"                ETC_LeastCostDirectedPath(1, SAMPLE_LIST_SIZE2, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][partition][0], 31, 31, 31, &pathing_params, ETC_RGBEXPANDVECTO5BIT, ETC_EVALUATEDELTAERRORGENERIC);\n"\
"                DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"            }\n"\
"        }\n"\
"        for (i = 0; i < 2; i++)\n"\
...
"                MEMBER(l0_quant, Y) = MEMBER(mean_quant[flipped][0], Y) + ip0;\n"\
"                MEMBER(l0_quant, Z) = MEMBER(mean_quant[flipped][0], Z) + ip0;\n"\
"                for (ip1 = min_index[flipped][1]; ip1 <= max_index[flipped][1]; ip1++) // TODO: this isn't dependent on ip0 in non-delta mode, it's N + N, not N*N\n"\
"                {\n"\
"                    uchar3 l1_quant;\n"\
"                    float local_error;\n"\
"\n"\
...
"                    MEMBER(l1_quant, Z) = MEMBER(mean_quant[flipped][1], Z) + ip1;\n"\
"\n"\
"                    local_error = ETC_EvaluateError(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(l0_quant), &g_partition_indexes[flipped][0][0]);\n"\
"                    local_error += ETC_EvaluateError(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(l1_quant), &g_partition_indexes[flipped][1][0]);\n"\
"\n"\
"                    if (local_error < fixed_err)\n"\
"                    {\n"\
"\n"\
...
"                int y;\n"\
"                int z;\n"\
"                int min_x = MEMBER(best_positions[partition], X) > 1 ? MEMBER(best_positions[partition], X) - 2 : 0;\n"\
"                int min_y = MEMBER(best_positions[partition], Y) > 1 ? MEMBER(best_positions[partition], Y) - 2 : 0;\n"\
"                int min_z = MEMBER(best_positions[partition], Z) > 1 ? MEMBER(best_positions[partition], Z) - 2 : 0;\n"\
"                int max_x = MEMBER(best_positions[partition], X) < 14 ? MEMBER(best_positions[partition], X) + 2 : 15;\n"\
"                int max_y = MEMBER(best_positions[partition], Y) < 14 ? MEMBER(best_positions[partition], Y) + 2 : 15;\n"\
"                int max_z = MEMBER(best_positions[partition], Z) < 14 ? MEMBER(best_positions[partition], Z) + 2 : 15;\n"\
"\n"\
"                for (z = min_z; z <= max_z; z++)\n"\
"                    for (y = min_y; y <= max_y; y++)\n"\
"                        for (x = min_x; x <= max_x; x++)\n"\
...
"                sample.error = ETC_EvaluateError(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), &g_partition_indexes[flipped][partition][0]);\n"\
"                DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"                ETC_LeastCostDirectedPath(1, 1, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][partition][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATEDELTAERRORGENERIC);\n"\
"                DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"            }\n"\
"        }\n"\
"        for (i = 0; i < 2; i++)\n"\
...
"                    sample.error = ETC_EvaluateErrorTSingle(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[flipped], set_count[flipped]);\n"\
"                    DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"                    ETC_LeastCostDirectedPath(1, 1, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][0][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATETSINGLEERRORGENERIC);\n"\
"                    DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"\n"\
"                    sample.pos_quantised = UCHAR3(0, 0, 0);\n"\
"                    DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
...
"                    sample.error = ETC_EvaluateErrorTTriple(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[(flipped + 1) % 2], set_count[(flipped + 1) % 2]);\n"\
"                    DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"                    ETC_LeastCostDirectedPath(1, 1, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][1][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATETTRIPLEERRORGENERIC);\n"\
"                    DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"                }\n"\
"                for (i = 0; i < 2; i++)\n"\
"                    best_dist[i] = sample_list[i][0][0].error + sample_list[i][1][0].error;\n"\
...
"                        sample.error = ETC_EvaluateErrorH(is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[flipped], set_count[flipped], table);\n"\
"                        DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"                        ETC_LeastCostDirectedPath(1, 1, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][table][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATEHERRORGENERIC);\n"\
"                        DEBUG_PRINTF((\"trans_pixel: 0x%p\\n\", trans_pixel));\n"\
"                    }\n"\
"                }\n"\
"\n"\
...
"        ETC_EncodeRGBFast(allowed_modes, is_etc2, is_ypbpr, pixel, block);\n"\
"    else\n"\
"        ETC_EncodeRGBQuality(allowed_modes, error_target, refine_search, is_etc2, is_ypbpr, pixel, block);\n"\
"}\n"\
"\n"\
"float ETC_EvaluateErrorAlpha(float *alpha, float base_alpha, float mul, int table)\n"\
"{\n"\
...
"                pathing_params.partition_indexes = &g_partition_indexes[flipped][partition][0];\n"\
"                sample.error = ETC_EvaluateErrorWeighted(is_ypbpr, trans_pixel, weight, ETC_RGBExpandVecTo5Bit(sample.pos_quantised), &g_partition_indexes[flipped][partition][0]);\n"\
"                ETC_LeastCostDirectedPathWeighted(1, SAMPLE_LIST_SIZE2, error_target, refine_search, is_ypbpr, trans_pixel, weight, sample, &sample_list[flipped][partition][0], 31, 31, 31, &pathing_params, ETC_RGBEXPANDVECTO5BIT, ETC_EVALUATEDELTAERRORGENERICWEIGHTED);\n"\
"            }\n"\
"        }\n"\
"\n"\
"        for (i = 0; i < 2; i++)\n"\
...
"                pathing_params.partition_indexes = &g_partition_indexes[flipped][partition][0];\n"\
"                sample.error = ETC_EvaluateErrorWeighted(is_ypbpr, trans_pixel, weight, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), &g_partition_indexes[flipped][partition][0]);\n"\
"                ETC_LeastCostDirectedPathWeighted(1, 1, error_target, refine_search, is_ypbpr, trans_pixel, weight, sample, &sample_list[flipped][partition][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATEDELTAERRORGENERICWEIGHTED);\n"\
"            }\n"\
"        }\n"\
"\n"\
"        for (i = 0; i < 2; i++)\n"\
...
"                    pathing_params.num_indexes = set_count[flipped];\n"\
"                    sample.error = ETC_EvaluateErrorTSingleWeighted(is_ypbpr, trans_pixel, weight, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[flipped], set_count[flipped]);\n"\
"                    ETC_LeastCostDirectedPathWeighted(1, 1, error_target, refine_search, is_ypbpr, trans_pixel, weight, sample, &sample_list[flipped][0][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATETSINGLEERRORGENERICWEIGHTED);\n"\
"\n"\
"                    sample.pos_quantised = UCHAR3(0, 0, 0);\n"\
"                    pathing_params.indexes = set_indexes[(flipped + 1) % 2];\n"\
"                    pathing_params.num_indexes = set_count[(flipped + 1) % 2];\n"\
"                    sample.error = ETC_EvaluateErrorTTripleWeighted(is_ypbpr, trans_pixel, weight, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[(flipped + 1) % 2], set_count[(flipped + 1) % 2]);\n"\
"                    ETC_LeastCostDirectedPathWeighted(1, 1, error_target, refine_search, is_ypbpr, trans_pixel, weight, sample, &sample_list[flipped][1][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATETTRIPLEERRORGENERICWEIGHTED);\n"\
"\n"\
"                }\n"\
"                for (i = 0; i < 2; i++)\n"\
"                    best_dist[i] = sample_list[i][0][0].error + sample_list[i][1][0].error;\n"\
...
"                        pathing_params.mode = table;\n"\
"                        sample.error = ETC_EvaluateErrorHWeighted(is_ypbpr, trans_pixel, weight, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[flipped], set_count[flipped], table);\n"\
"                        ETC_LeastCostDirectedPathWeighted(1, 1, error_target, refine_search, is_ypbpr, trans_pixel, weight, sample, &sample_list[flipped][table][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATEHERRORGENERICWEIGHTED);\n"\
"                    }\n"\
"                }\n"\
"\n"\
"                best_dist[0] = sample_list[0][0][0].error + sample_list[1][0][0].error;\n"\
...
"                pathing_params.partition_indexes = &g_partition_indexes[flipped][partition][0];\n"\
"                sample.error = ETC_EvaluateErrorA1(transparency, is_ypbpr, trans_pixel, ETC_RGBExpandVecTo5Bit(sample.pos_quantised), &g_partition_indexes[flipped][partition][0]);\n"\
"                ETC_LeastCostDirectedPathA1(1, SAMPLE_LIST_SIZE2, transparency, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][partition][0], 31, 31, 31, &pathing_params, ETC_RGBEXPANDVECTO5BIT, ETC_EVALUATEDELTAERRORGENERICA1);\n"\
"            }\n"\
"        }\n"\
"\n"\
"        for (i = 0; i < 2; i++)\n"\
...
"                pathing_params.num_indexes = set_count[flipped];\n"\
"                sample.error = ETC_EvaluateErrorTSingleA1(transparency, is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[flipped], set_count[flipped]);\n"\
"                ETC_LeastCostDirectedPathA1(1, 1, transparency, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][0][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATETSINGLEERRORGENERICA1);\n"\
"\n"\
"                sample.pos_quantised = UCHAR3(0, 0, 0);\n"\
"                pathing_params.indexes = set_indexes[(flipped + 1) % 2];\n"\
"                pathing_params.num_indexes = set_count[(flipped + 1) % 2];\n"\
"                sample.error = ETC_EvaluateErrorTTripleA1(transparency, is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[(flipped + 1) % 2], set_count[(flipped + 1) % 2]);\n"\
"                ETC_LeastCostDirectedPathA1(1, 1, transparency, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][1][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATETTRIPLEERRORGENERICA1);\n"\
"            }\n"\
"            for (i = 0; i < 2; i++)\n"\
"                best_dist[i] = sample_list[i][0][0].error + sample_list[i][1][0].error;\n"\
"\n"\
...
"                    pathing_params.h_index = flipped;\n"\
"                    sample.error = ETC_EvaluateErrorHA1(transparency, is_ypbpr, trans_pixel, ETC_RGBExpandVecTo4Bit(sample.pos_quantised), set_indexes[flipped], set_count[flipped], table, flipped);\n"\
"                    ETC_LeastCostDirectedPathA1(1, 1, transparency, error_target, refine_search, is_ypbpr, trans_pixel, sample, &sample_list[flipped][table][0], 15, 15, 15, &pathing_params, ETC_RGBEXPANDVECTO4BIT, ETC_EVALUATEHERRORGENERICA1);\n"\
"                }\n"\
"            }\n"\
"\n"\
"            best_dist[0] = sample_list[0][0][0].error + sample_list[1][0][0].error;\n"\
...
"        ETC_EncodeRGBA1WeightedFast(allowed_modes, is_ypbpr, alpha_cutoff, pixel, weight, block);\n"\
"    else\n"\
"        ETC_EncodeRGBA1WeightedQuality(allowed_modes, error_target, refine_search, is_ypbpr, alpha_cutoff, pixel, weight, block);\n"\
"}\n"\
ETC_EPILOG
#endif